#!/usr/bin/python
# -*- coding: utf-8 -*-
import xbmc, xbmcplugin, xbmcgui, xbmcaddon, xbmcvfs, locale, sys, urllib, urllib2, re, os, datetime, base64
...
if useAlternatePlaylistPath=="true":
  playListFile=xbmc.translatePath(settings.getSetting("alternatePlDir2")+"/SimplePlaylists.spl")
  playListNames=xbmc.translatePath(settings.getSetting("alternatePlDir2")+"/playlists")
  playListSubNames=xbmc.translatePath(settings.getSetting("alternatePlDir2")+"/subfolders")
else:
...
myPlaylists=[]
if xbmcvfs.exists(playListNames):
  fh = xbmcvfs.File(playListNames)
        ...
        for line in lines:
          if mode=="removePlaylist":
            ...
            if line.find("###PLAYLIST###="+name)==-1:
               newContent+=line
        fh.close()
        fh=xbmcvfs.File(playListFile, 'w')
        fh.write(newContent)
        fh.close()
            ...
            if xbmcvfs.exists(playListNames):
              ...
              if nr>=0:
                ...
                for line in lines:
                  ...
                  fullTemp+=temp
                fh.close()
                fh=xbmcvfs.File(playListNames, 'w')
                fh.write(fullTemp)
                fh.close()
            else:
              xbmc.executebuiltin('XBMC.Notification(Info:,'+str(translation(30023))+'!,5000)')
          elif type==translation(30027):
            if xbmcvfs.exists(playListSubNames):
              pls=[]
              fh = xbmcvfs.File(playListSubNames)
              lines = fh.read().splitlines()
              for line in lines:
              ...
              if nr>=0:
                ...
                for line in lines:
                  ...
                  fullTemp+=temp
                fh.close()
                fh=xbmcvfs.File(playListSubNames, 'w')
                fh.write(fullTemp)
                fh.close()
            else:
...
def showPlaylists():
        ...
        if nr >=0:
          ...
          if plType==translation(30001):
            xbmc.executebuiltin('XBMC.ActivateWindow(10025,plugin://script.simpleplaylists/?mode=playListMain&content_type=Video)')
          elif plType==translation(30002):
            xbmc.executebuiltin('XBMC.ActivateWindow(10502,plugin://script.simpleplaylists/?mode=playListMain&content_type=Audio)')
          elif plType==translation(30003):
            xbmc.executebuiltin('XBMC.ActivateWindow(10002,plugin://script.simpleplaylists/?mode=playListMain&content_type=Image)')
...
def addCurrentUrl():
        ...
        #isDir = xbmc.getInfoLabel('ListItem.IsFolder')
        isDir = False
                ...
                if kb.isConfirmed():
                  ...
                  if xbmcvfs.exists(playListNames):
                    ...
                    if content.find(plType+"=")>=0:
                      ...
                      fh = xbmcvfs.File(playListNames, 'a')
                    ...
                    if kb.isConfirmed():
                      ...
                      if xbmcvfs.exists(playListSubNames):
                        ...
                        if content.find(plForCat)>=0:
                          ...
                          fh = xbmcvfs.File(playListSubNames, 'a')
              ...
              if pl!="" and pl!="- "+translation(30017) and pl!=translation(30019) and pl!="- "+translation(30019):
                ...
                if xbmcvfs.exists(playListFile):
                  ...
                  if content.find(playlistEntry[playlistEntry.find("###URL###="):])==-1:
                    fh=xbmcvfs.File(playListFile, 'w')
                    fh.write(content+playlistEntry+"\n")
                    fh.close()
                  else:
        ...
        if xbmcvfs.exists(playListFile):
          ...
          for line in lines:
            ...
            if not pl in playlists:
              if pl.find(contentType)==0:
                  playlists.append(pl)
          fh.close()
          for pl in playlists:
            titleNew=pl;
            if contentType!="":
              titleNew=pl.replace(contentType+": ","")
            if xbmcvfs.exists(playListSubNames):
              fh = xbmcvfs.File(playListSubNames)
              content=fh.read()
              fh.close()
...
contentType = urllib.unquote_plus(params.get('content_type', '')).title()
